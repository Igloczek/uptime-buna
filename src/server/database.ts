...
class Database {
    ...
    static knexMigrationsPath = "./src/db/knex_migrations";
    ...
    /**
     * Patch the database
     * @param {number} _port Start the migration server for aggregate tables on this port if provided
     * @param {string} _hostname Start the migration server for aggregate tables on this hostname if provided
     * @returns {Promise<void>}
     */
    static async patch(_port = undefined, _hostname = undefined) {
        log.info("db", "Using Bun-native SQLite bootstrap; legacy Redbean/Knex migrations are skipped on this path.");
    }
        ...
        if (version === this.latestVersion) {
            ...
            try {
                for (let i = version + 1; i <= this.latestVersion; i++) {
                    const sqlFile = `./src/db/old_migrations/patch${i}.sql`;
    ...
    static async patch2Recursion(sqlFilename, databasePatchedFiles) {
        ...
        if (!databasePatchedFiles[sqlFilename]) {
            ...
            this.patched = true;
            await this.importSQLFile("./src/db/old_migrations/" + sqlFilename);
