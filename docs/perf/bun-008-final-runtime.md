...
- Docker runtime target at the time: `docker build -f Dockerfile -t pocketkuma:bun-final --target release .`
- Current Docker runtime target: `docker build -f Dockerfile -t pocketkuma:local .`
...
- Inherited Docker helper scripts and compose defaults now target `pocketkuma:*` images instead of upstream `louislam/uptime-kuma:*` images. The root `compose.yaml` builds this repository with `Dockerfile` instead of pulling a public upstream runtime image.
...
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `express`                                   | `src/server/server.ts`, setup flow, routers, static middleware, and `PocketKumaServer.app` still mount inherited Express handlers. Bun serves the default listener, but route extraction is incomplete. | Extract remaining Express routers/middleware into Bun-native fetch handlers, then delete Express.         |
...
- Image: `pocketkuma:bun-final`
- Size: `478563803` bytes.
- Build: passed with `docker compose --progress=plain -p pocketkuma-smoke -f compose.yaml up -d --build --force-recreate`.
