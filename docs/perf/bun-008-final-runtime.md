...
- Compose/container smoke: passed with `UPTIME_BUNA_PORT=3011 UPTIME_BUNA_DATA_DIR=/tmp/uptime-buna-compose-smoke docker compose -p uptime-buna-smoke -f compose.yaml up -d --build --force-recreate`. The container was healthy, served the app through Bun 1.2.17, completed setup/login, created an HTTP monitor for `http://127.0.0.1:3001`, and stored a heartbeat with `status = 1`, `msg = '200 - OK'`, and `ping = 3`.
